...
const ExploreCard = ({active, handleClick, index, id, imgUrl, title }) => (
  <motion.div 
      className={`flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-[0.7s] cursor-pointer ease-out-flex relative ${active === id ? 'lg:flex-[3.5] flex-[10]' : 'flex-[2] lg:flex-[0.5]'}`}
      onClick={()=>handleClick(id)}
      variants={fadeIn('left', 'sping', index * 0.5, 0.75)}>
    ...
    (
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        <div className={`${styles.flexCenter} w-[60px] h-[40px] md:h-[60px] rounded-[24px] glassmorphism mb-[16px]`}>
